...
public class CashewToMonoCSVConverter {
    ...
    public static Cashew monoCSVToCashew(MonoCSV monoCSV, String acount) {
        Cashew cashew = new Cashew();
        cashew.setAccount(acount);
        ...
        cashew.setCategoryName(""); // Категорію потрібно визначити додатково
    ...
    public static List<Cashew> convertToCashewList(List<MonoCSV> operationDTOList, String acount) {
        List<Cashew> cashewList = new ArrayList<>();
        for (MonoCSV operationDTO : operationDTOList) {
            cashewList.add(monoCSVToCashew(operationDTO, acount));
        }
        return cashewList;
    }
